...
instance (Monoid a, SingI n) => Monoid (Vec a n) where
  {-# INLINE mempty #-}
...
tightenSigma :: Sigma Nat (TyCon1 (G.Vec V.Vector a)) -> Sigma Nat (TyCon1 (Vec a))
{-# INLINE tightenSigma #-}
tightenSigma = coerce

tightenSigmaM :: m (Sigma Nat (TyCon1 (G.Vec V.Vector a))) -> m (Sigma Nat (TyCon1 (Vec a)))
{-# INLINE tightenSigmaM #-}
tightenSigmaM = unsafeCoerce

tightenSigma2 :: (Sigma Nat (TyCon1 (G.Vec V.Vector a)), Sigma Nat (TyCon1 (G.Vec V.Vector a))) -> (Sigma Nat (TyCon1 (Vec a)), Sigma Nat (TyCon1 (Vec a)))
...
indexM :: (Monad m) => Vec a n -> Bound n -> m a
{-# INLINE indexM #-}
...
take :: SNat i -> Vec a j -> Vec a (Min i j)
{-# INLINE take #-}
...
replicateM' :: (Monad m, SingI n) => m a -> m (Vec a n)
{-# INLINE replicateM' #-}
replicateM' x = tightenMonad (G.replicateM' x)

generate :: SNat n -> (Bound n -> a) -> Vec a n
{-# INLINE generate  #-}
generate amount f = Vec (G.generate amount f)

generate' :: (SingI n) => (Bound n -> a) -> Vec a n
{-# INLINE generate' #-}
generate' f = Vec (G.generate' f)

generateM :: (Monad m) => SNat n -> (Bound n -> m a) -> m (Vec a n)
{-# INLINE generateM #-}
generateM amount f = tightenMonad (G.generateM amount f)

generateM' :: (Monad m, SingI n) => (Bound n -> m a) -> m (Vec a n)
{-# INLINE generateM' #-}
...
iterate' :: (SingI n) => (a -> a) -> a -> Vec a n
{-# INLINE iterate' #-}
iterate' f x = Vec (G.iterate' f x)

-- create

unfoldr :: (b -> Maybe (a,b)) -> b -> Sigma Nat (TyCon1 (Vec a))
{-# INLINE unfoldr #-}
...
construct' :: (SingI n) => (forall m. Vec a m -> a) -> Vec a n
{-# INLINE construct' #-}
...
constructr' :: (SingI n) => (forall m. Vec a m -> a) -> Vec a n
{-# INLINE constructr' #-}
...
enumFromN' :: (SingI n, Num a) => a -> Vec a n
{-# INLINE enumFromN' #-}
...
enumFromStepN' :: (SingI n, Num a) => a -> a -> Vec a n
{-# INLINE enumFromStepN' #-}
...
(//) :: Vec a n -> [(Bound n, a)] -> Vec a n
{-# INLINE (//) #-}
(Vec v) // us = Vec (v G.// us)

update :: Vec a n -> Vec (Bound n, a) m -> Vec a n
{-# INLINE update #-}
update (Vec v) (Vec w) = Vec (G.update v w)

update_ :: Vec a n -> Vec (Bound n) m -> Vec a m -> Vec a n
{-# INLINE update_ #-}
update_ (Vec v) (Vec w) (Vec x) = Vec (G.update_ v w x)

accum :: (a -> b -> a) -> Vec a n -> [(Bound n, b)] -> Vec a n
{-# INLINE accum #-}
accum f (Vec v) us = Vec (G.accum f v us)

accumulate :: (a -> b -> a) -> Vec a n -> Vec (Bound n, b) m -> Vec a n
{-# INLINE accumulate #-}
accumulate f (Vec v) (Vec w) = Vec (G.accumulate f v w)

accumulate_ :: (a -> b -> a) -> Vec a n -> Vec (Bound n) m -> Vec b m -> Vec a n
{-# INLINE accumulate_ #-}
...
backpermute :: Vec a n -> Vec (Bound n) m -> Vec a m
{-# INLINE backpermute #-}
backpermute (Vec v) (Vec w) = Vec (G.backpermute v w)

indexed :: Vec a n -> Vec (Bound n, a) n
{-# INLINE indexed #-}
indexed (Vec v) = Vec (G.indexed v)

diagonal :: (SingI n) => Vec (Vec a n) n -> Vec a n
{-# INLINE diagonal #-}
...
imap :: (Bound n -> a -> b) -> Vec a n -> Vec b n
{-# INLINE imap #-}
...
imapM :: (Monad m) => (Bound n -> a -> m b) -> Vec a n -> m (Vec b n)
{-# INLINE imapM #-}
...
imapM_ :: (Monad m) => (Bound n -> a -> m b) -> Vec a n -> m ()
{-# INLINE imapM_ #-}
...
filter :: (a -> Bool) -> Vec a n -> Sigma Nat (TyCon1 (Vec a))
{-# INLINE filter #-}
filter f (Vec v) = tightenSigma (G.filter f v)

ifilter :: (Bound n -> a -> Bool) -> Vec a n -> Sigma Nat (TyCon1 (Vec a))
{-# INLINE ifilter #-}
ifilter f (Vec v) = tightenSigma (G.ifilter f v)

filterM :: (Monad m) => (a -> m Bool) -> Vec a n -> m (Sigma Nat (TyCon1 (Vec a)))
{-# INLINE filterM #-}
filterM f (Vec v) = tightenSigmaM (G.filterM f v)

takeWhile :: (a -> Bool) -> Vec a n -> Sigma Nat (TyCon1 (Vec a))
{-# INLINE takeWhile #-}
takeWhile f (Vec v) = tightenSigma (G.takeWhile f v)

dropWhile :: (a -> Bool) -> Vec a n -> Sigma Nat (TyCon1 (Vec a))
{-# INLINE dropWhile #-}
dropWhile f (Vec v) = tightenSigma (G.dropWhile f v)

partition :: (a -> Bool) -> Vec a n -> (Sigma Nat (TyCon1 (Vec a)), Sigma Nat (TyCon1 (Vec a)))
{-# INLINE partition #-}
partition f (Vec v) = tightenSigma2 (G.partition f v)

unstabelPartition :: (a -> Bool) -> Vec a n -> (Sigma Nat (TyCon1 (Vec a)), Sigma Nat (TyCon1 (Vec a)))
{-# INLINE unstabelPartition #-}
unstabelPartition f (Vec v) = tightenSigma2 (G.unstablePartition f v)

span :: (a -> Bool) -> Vec a n -> (Sigma Nat (TyCon1 (Vec a)), Sigma Nat (TyCon1 (Vec a)))
{-# INLINE span #-}
span f (Vec v) = tightenSigma2 (G.span f v)

break :: (a -> Bool) -> Vec a n -> (Sigma Nat (TyCon1 (Vec a)), Sigma Nat (TyCon1 (Vec a)))
{-# INLINE break #-}
...
findIndex :: (a -> Bool) -> Vec a n -> Maybe (Bound n)
{-# INLINE findIndex #-}
findIndex f (Vec v) = G.findIndex f v

findIndices :: (a -> Bool) -> Vec a n -> Sigma Nat (TyCon1 (Vec (Bound n)))
{-# INLINE findIndices #-}
findIndices f (Vec v) = tightenSigma (G.findIndices f v)

elemIndex :: (Eq a) => a -> Vec a n -> Maybe (Bound n)
{-# INLINE elemIndex #-}
elemIndex x (Vec v) = G.elemIndex x v

elemIndices :: (Eq a) => a -> Vec a n -> Sigma Nat (TyCon1 (Vec (Bound n)))
{-# INLINE elemIndices #-}
...
ifoldl :: (a -> Bound n -> b -> a) -> a -> Vec b n -> a
{-# INLINE ifoldl #-}
...
ifoldlD :: (a l -> Bound n -> b -> a (S l)) -> a Z -> Vec b n -> a n
{-# INLINE ifoldlD #-}
...
ifoldr :: (Bound n -> a -> b -> b) -> b -> Vec a n -> b
{-# INLINE ifoldr #-}
...
ifoldrD :: (Bound n -> a -> b l -> b (S l)) -> b Z -> Vec a n -> b n
{-# INLINE ifoldrD #-}
...
minIndex :: (Ord a) => Vec a (S n) -> Bound (S n)
{-# INLINE minIndex #-}
minIndex (Vec v) = G.minIndex v

minIndexBy :: (a -> a -> Ordering) -> Vec a (S n) -> Bound (S n)
{-# INLINE minIndexBy #-}
minIndexBy c (Vec v) = G.minIndexBy c v

maxIndex :: (Ord a) => Vec a (S n) -> Bound (S n)
{-# INLINE maxIndex #-}
maxIndex (Vec v) = G.maxIndex v

maxIndexBy :: (a -> a -> Ordering) -> Vec a (S n) -> Bound (S n)
{-# INLINE maxIndexBy #-}
...
fromList :: [a] -> Sigma Nat (TyCon1 (Vec a))
{-# INLINE fromList #-}
...
fromListN' :: (SingI n) => [a] -> Maybe (Vec a n)
{-# INLINE fromListN' #-}
...
fromVector :: V.Vector a -> Sigma Nat (TyCon1 (Vec a))
{-# INLINE fromVector #-}
...
fromVectorN' :: (SingI n) => V.Vector a -> Maybe (Vec a n)
{-# INLINE fromVectorN' #-}
